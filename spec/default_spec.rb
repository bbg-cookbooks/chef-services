...
describe 'services::default' do
  ...
  context "when the services attribute is an empty array" do
    before(:each) do
      chef.node.set['services'] = Hash.new
  ...
  context "when life is messy and we are stopping many services" do
    before(:each) do
      chef.node.set['services'] = { :cups => [:stop], :bluetooth => [:stop] }
      chef.converge 'services::default'
    end

    it 'should stop the service' do
      chef.should stop_service 'cups'
      chef.should stop_service 'bluetooth'
    end
  end
